"""
One call that turns a bill image / PDF into structured data: hybrid OCR (English + Marathi) -> fields -> address -> confidence.

    analysis = analyze_bill(path, budget_seconds=60)

Returns a plain dict (JSON-friendly). Never raises for OCR trouble: `error` is set instead, and the caller can fall back to the
classic pipeline. The confidence reflects EVIDENCE that the fields are right (labels found, passes agreeing, structure valid), not just
Tesseract's per-word certainty, which is low on small photographed bills even when every field is correct.
"""
import logging
import time
...
def reading_text(lines: List[Line]) -> str:
    """
    A human-readable transcript for storage / display: the single OCR pass that read the most, in page order.
    (All passes together contain every line several times over, which is useful for voting but unreadable.)
    """
...
def analyze_bill(path: str, budget_seconds: float = 60.0) -> Dict[str, Any]:
    ...
    result = {
        "error": None,
        "text": reading_text(ocr.lines),
        "ocr_confidence": ocr.confidence,
        "ocr_engine": f"hybrid({ocr.languages})",
